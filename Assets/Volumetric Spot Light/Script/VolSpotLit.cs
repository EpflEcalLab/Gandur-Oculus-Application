...
public class VolSpotLit : MonoBehaviour
{
	...
	public GameObject m_LightVolumetric;
	...
	void Awake ()
	{
		...
		//if (litvol != null)
		{
			m_LitVolRdr = m_LightVolumetric.GetComponent<MeshRenderer> ();
			m_LitVolTransform =m_LightVolumetric.GetComponent<Transform> ();
			m_LitVolTaper = m_LightVolumetric.GetComponent<Taper> ();
		}
	...
	void Update ()
    {
		...
		//if (litsrc != null)
		{
			...
			Vector3 np = new Vector3 (m_LightSource.transform.position.x, m_LitVolTaper.m_Height, m_LightSource.transform.position.z);
			m_LightSource.transform.position = np;
			m_LitVolRdr.material.SetVector ("_SpotPosition", m_LightSource.transform.position);
